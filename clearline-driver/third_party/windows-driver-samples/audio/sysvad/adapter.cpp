...
#include "clearline_ringbuffer.h"
...
#define CLEARLINE_CONTROL_DEVICE_NAME      L"\\Device\\ClearLineControl"
#define CLEARLINE_CONTROL_DOS_SYMBOLIC_LINK L"\\DosDevices\\ClearLineControl"

PDEVICE_OBJECT gClearLineControlDeviceObject = NULL;
PDRIVER_DISPATCH gClearLineSavedCreateDispatch = NULL;
PDRIVER_DISPATCH gClearLineSavedCloseDispatch = NULL;
PDRIVER_DISPATCH gClearLineSavedDeviceControlDispatch = NULL;
KSPIN_LOCK gClearLinePcmRingBufferLock;
PUCHAR gClearLinePcmRingBuffer = NULL;
ULONG gClearLinePcmRingBufferCapacity = 0;
ULONG gClearLinePcmRingBufferReadOffset = 0;
ULONG gClearLinePcmRingBufferWriteOffset = 0;
ULONG gClearLinePcmRingBufferReadableBytes = 0;
ULONGLONG gClearLinePcmTotalWrittenBytes = 0;
ULONGLONG gClearLinePcmTotalDroppedBytes = 0;
ULONGLONG gClearLinePcmOverflowCount = 0;
ULONGLONG gClearLinePcmTotalReadBytes = 0;
ULONGLONG gClearLinePcmTotalUnderrunBytes = 0;
ULONGLONG gClearLinePcmUnderrunCount = 0;
...
DRIVER_DISPATCH ClearLineControlDispatchCreateClose;

_Dispatch_type_(IRP_MJ_CLOSE)
DRIVER_DISPATCH ClearLineControlDispatchCreateClose;

_Dispatch_type_(IRP_MJ_DEVICE_CONTROL)
DRIVER_DISPATCH ClearLineControlDispatchDeviceControl;

NTSTATUS
ClearLineCreateControlDevice(
    _In_ PDRIVER_OBJECT DriverObject
);

void
ClearLineDestroyControlDevice();
...
ClearLineForwardIrp(
    ...
    _Inout_ PIRP Irp,
    _In_opt_ PDRIVER_DISPATCH SavedDispatch
)
{
    if (SavedDispatch != NULL)
    {
        return SavedDispatch(DeviceObject, Irp);
    }
...
ClearLineInitializeRingBuffer()
{
    if (gClearLinePcmRingBuffer != NULL)
    {
        return STATUS_SUCCESS;
    }

    KeInitializeSpinLock(&gClearLinePcmRingBufferLock);

    gClearLinePcmRingBuffer =
        static_cast<PUCHAR>(ExAllocatePool2(
            POOL_FLAG_NON_PAGED,
            CLEARLINE_PCM_RING_BUFFER_BYTES,
            CLEARLINE_POOL_TAG));
    if (gClearLinePcmRingBuffer == NULL)
    {
    ...
    RtlZeroMemory(gClearLinePcmRingBuffer, CLEARLINE_PCM_RING_BUFFER_BYTES);
    gClearLinePcmRingBufferCapacity = CLEARLINE_PCM_RING_BUFFER_BYTES;
    gClearLinePcmRingBufferReadOffset = 0;
    gClearLinePcmRingBufferWriteOffset = 0;
    gClearLinePcmRingBufferReadableBytes = 0;
    gClearLinePcmTotalWrittenBytes = 0;
    gClearLinePcmTotalDroppedBytes = 0;
    gClearLinePcmOverflowCount = 0;
    gClearLinePcmTotalReadBytes = 0;
    gClearLinePcmTotalUnderrunBytes = 0;
    gClearLinePcmUnderrunCount = 0;
...
ClearLineDestroyRingBuffer()
{
    PUCHAR bufferToFree = NULL;

    if (gClearLinePcmRingBuffer != NULL)
    {
        KIRQL oldIrql;
        KeAcquireSpinLock(&gClearLinePcmRingBufferLock, &oldIrql);
        bufferToFree = gClearLinePcmRingBuffer;
        gClearLinePcmRingBuffer = NULL;
        gClearLinePcmRingBufferCapacity = 0;
        gClearLinePcmRingBufferReadOffset = 0;
        gClearLinePcmRingBufferWriteOffset = 0;
        gClearLinePcmRingBufferReadableBytes = 0;
        gClearLinePcmTotalWrittenBytes = 0;
        ...
        gClearLinePcmTotalUnderrunBytes = 0;
        gClearLinePcmUnderrunCount = 0;
        KeReleaseSpinLock(&gClearLinePcmRingBufferLock, oldIrql);
    }

    if (bufferToFree != NULL)
    {
        ExFreePoolWithTag(bufferToFree, CLEARLINE_POOL_TAG);
    }
}
...
ClearLineGetRingBufferReadableBytes()
{
    ULONG readableBytes = 0;

    if (gClearLinePcmRingBuffer != NULL)
    {
        KIRQL oldIrql;
        KeAcquireSpinLock(&gClearLinePcmRingBufferLock, &oldIrql);
        readableBytes = gClearLinePcmRingBufferReadableBytes;
        KeReleaseSpinLock(&gClearLinePcmRingBufferLock, oldIrql);
    }
...
{
    if (BytesAccepted != NULL)
    {
        *BytesAccepted = 0;
    }

    if ((BytesToWrite % sizeof(SHORT)) != 0 || BytesToWrite > CLEARLINE_MAX_PCM_WRITE_BYTES)
    {
    ...
    if (gClearLinePcmRingBuffer == NULL || gClearLinePcmRingBufferCapacity == 0)
    {
        KeReleaseSpinLock(&gClearLinePcmRingBufferLock, oldIrql);
        return STATUS_DEVICE_NOT_READY;
    }

    ULONG bytesToCopy = BytesToWrite;
    ULONG overflowBytes =
        (gClearLinePcmRingBufferReadableBytes + bytesToCopy > gClearLinePcmRingBufferCapacity)
            ? (gClearLinePcmRingBufferReadableBytes + bytesToCopy - gClearLinePcmRingBufferCapacity)
            : 0;
    if (overflowBytes > 0)
    {
        gClearLinePcmRingBufferReadOffset =
            (gClearLinePcmRingBufferReadOffset + overflowBytes) % gClearLinePcmRingBufferCapacity;
        gClearLinePcmRingBufferReadableBytes -= overflowBytes;
        gClearLinePcmTotalDroppedBytes += overflowBytes;
        gClearLinePcmOverflowCount++;
    }

    ULONG firstCopyBytes = bytesToCopy;
    ULONG bytesUntilWrap = gClearLinePcmRingBufferCapacity - gClearLinePcmRingBufferWriteOffset;
    if (firstCopyBytes > bytesUntilWrap)
    {
        firstCopyBytes = bytesUntilWrap;
    }

    RtlCopyMemory(
        gClearLinePcmRingBuffer + gClearLinePcmRingBufferWriteOffset,
        Source,
        firstCopyBytes);

    ULONG remainingBytes = bytesToCopy - firstCopyBytes;
    if (remainingBytes > 0)
    {
        RtlCopyMemory(
            gClearLinePcmRingBuffer,
    ...
    gClearLinePcmRingBufferWriteOffset =
        (gClearLinePcmRingBufferWriteOffset + bytesToCopy) % gClearLinePcmRingBufferCapacity;
    gClearLinePcmRingBufferReadableBytes += bytesToCopy;
    gClearLinePcmTotalWrittenBytes += bytesToCopy;

    if (BytesAccepted != NULL)
    {
        *BytesAccepted = bytesToCopy;
    }
...
)
{
    ...
    ULONG bytesToCopy = min(BytesToRead, gClearLinePcmRingBufferReadableBytes);
    if (bytesToCopy > 0)
    {
        ULONG firstCopyBytes = bytesToCopy;
        ULONG bytesUntilWrap = gClearLinePcmRingBufferCapacity - gClearLinePcmRingBufferReadOffset;
        if (firstCopyBytes > bytesUntilWrap)
        {
            firstCopyBytes = bytesUntilWrap;
        }
        ...
        RtlCopyMemory(
            Destination,
            gClearLinePcmRingBuffer + gClearLinePcmRingBufferReadOffset,
            firstCopyBytes);
        ...
        {
            RtlCopyMemory(
                Destination + firstCopyBytes,
                gClearLinePcmRingBuffer,
                remainingBytes);
        }

        gClearLinePcmRingBufferReadOffset =
            (gClearLinePcmRingBufferReadOffset + bytesToCopy) % gClearLinePcmRingBufferCapacity;
        gClearLinePcmRingBufferReadableBytes -= bytesToCopy;
        gClearLinePcmTotalReadBytes += bytesToCopy;
    }
...
void
ClearLineRecordCaptureUnderrun(
    _In_ ULONG MissingBytes
)
{
    if (MissingBytes == 0)
    {
    ...
    if (gClearLinePcmRingBuffer != NULL)
    {
        gClearLinePcmTotalUnderrunBytes += MissingBytes;
        gClearLinePcmUnderrunCount++;
    }
    KeReleaseSpinLock(&gClearLinePcmRingBufferLock, oldIrql);
}
...
)
{
    ...
    ULONG bytesRead = 0;
    NTSTATUS ntStatus = ClearLineReadPcmFromRingBuffer(Destination, BytesToFill, &bytesRead);
    ...
    {
        ULONG missingBytes = BytesToFill - bytesRead;
        RtlZeroMemory(Destination + bytesRead, missingBytes);
        ClearLineRecordCaptureUnderrun(missingBytes);
    }
}
...
{
    RtlZeroMemory(Status, sizeof(ClearLineBufferStatus));

    if (gClearLinePcmRingBuffer != NULL)
    {
        KIRQL oldIrql;
        KeAcquireSpinLock(&gClearLinePcmRingBufferLock, &oldIrql);
        Status->CapacityBytes = gClearLinePcmRingBufferCapacity;
        Status->ReadableBytes = gClearLinePcmRingBufferReadableBytes;
        Status->WritableBytes =
            gClearLinePcmRingBufferCapacity - gClearLinePcmRingBufferReadableBytes;
        Status->TotalWrittenBytes = gClearLinePcmTotalWrittenBytes;
        Status->TotalDroppedBytes = gClearLinePcmTotalDroppedBytes;
        Status->OverflowCount = gClearLinePcmOverflowCount;
        Status->TotalReadBytes = gClearLinePcmTotalReadBytes;
        Status->TotalUnderrunBytes = gClearLinePcmTotalUnderrunBytes;
        Status->UnderrunCount = gClearLinePcmUnderrunCount;
        KeReleaseSpinLock(&gClearLinePcmRingBufferLock, oldIrql);
    }
}
...
)
{
    ...
    RtlInitUnicodeString(&deviceName, CLEARLINE_CONTROL_DEVICE_NAME);
    RtlInitUnicodeString(&symbolicLink, CLEARLINE_CONTROL_DOS_SYMBOLIC_LINK);

    if (gClearLineControlDeviceObject != NULL)
    {
        return STATUS_SUCCESS;
    }

    ntStatus = ClearLineInitializeRingBuffer();
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("ClearLineInitializeRingBuffer failed, 0x%x", ntStatus)),
        Exit);
    ...
    ntStatus = IoCreateDevice(
        ...
        FALSE,
        &gClearLineControlDeviceObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("ClearLine IoCreateDevice failed, 0x%x", ntStatus)),
        Exit);

    gClearLineControlDeviceObject->Flags |= DO_BUFFERED_IO;
    gClearLineControlDeviceObject->Flags &= ~DO_DEVICE_INITIALIZING;

    ntStatus = IoCreateSymbolicLink(&symbolicLink, &deviceName);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("ClearLine IoCreateSymbolicLink failed, 0x%x", ntStatus)),
        Exit);

    DPF(D_TERSE, ("ClearLine control device created"));
...
ClearLineDestroyControlDevice()
{
    ...
    if (gClearLineControlDeviceObject != NULL)
    {
        IoDeleteDevice(gClearLineControlDeviceObject);
        gClearLineControlDeviceObject = NULL;
    }
...
#pragma code_seg("PAGE")
NTSTATUS
ClearLineControlDispatchCreateClose(
    _In_ PDEVICE_OBJECT DeviceObject,
    _Inout_ PIRP Irp
)
{
    ...
    if (DeviceObject != gClearLineControlDeviceObject)
    {
        IO_STACK_LOCATION* stack = IoGetCurrentIrpStackLocation(Irp);
        PDRIVER_DISPATCH savedDispatch =
            stack->MajorFunction == IRP_MJ_CREATE
                ? gClearLineSavedCreateDispatch
                : gClearLineSavedCloseDispatch;
        return ClearLineForwardIrp(DeviceObject, Irp, savedDispatch);
    }

    ClearLineCompleteIrp(Irp, STATUS_SUCCESS, 0);
    return STATUS_SUCCESS;
}
...
)
{
    ...
    if (DeviceObject != gClearLineControlDeviceObject)
    {
        return ClearLineForwardIrp(DeviceObject, Irp, gClearLineSavedDeviceControlDispatch);
    }
    ...
    {
    case IOCTL_CLEARLINE_PING:
    {
        if (stack->Parameters.DeviceIoControl.OutputBufferLength < sizeof(ClearLinePingResponse))
        {
        ...
        ClearLinePingResponse* response =
            static_cast<ClearLinePingResponse*>(Irp->AssociatedIrp.SystemBuffer);
        response->Magic = CLEARLINE_PING_MAGIC;
        response->Version = CLEARLINE_PING_VERSION;
        response->SampleRateHz = 48000;
        response->Channels = 1;
        information = sizeof(ClearLinePingResponse);
        ntStatus = STATUS_SUCCESS;
        break;
    }

    case IOCTL_CLEARLINE_WRITE_PCM:
    {
        if (Irp->AssociatedIrp.SystemBuffer == NULL)
        {
            ntStatus = STATUS_INVALID_USER_BUFFER;
        ...
        ULONG bytesAccepted = 0;
        ntStatus = ClearLineWritePcmToRingBuffer(
            static_cast<PUCHAR>(Irp->AssociatedIrp.SystemBuffer),
    ...
    case IOCTL_CLEARLINE_GET_BUFFER_STATUS:
    {
        if (stack->Parameters.DeviceIoControl.OutputBufferLength < sizeof(ClearLineBufferStatus))
        {
            ntStatus = STATUS_BUFFER_TOO_SMALL;
            break;
        }
        ...
        ClearLineBufferStatus* status =
            static_cast<ClearLineBufferStatus*>(Irp->AssociatedIrp.SystemBuffer);
        ClearLineFillBufferStatus(status);
        information = sizeof(ClearLineBufferStatus);
        ntStatus = STATUS_SUCCESS;
        break;
    }
    ...
    ClearLineCompleteIrp(Irp, ntStatus, information);
    return ntStatus;
}
...
--*/
    ...
    gClearLineSavedCreateDispatch = DriverObject->MajorFunction[IRP_MJ_CREATE];
    gClearLineSavedCloseDispatch = DriverObject->MajorFunction[IRP_MJ_CLOSE];
    gClearLineSavedDeviceControlDispatch = DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL];

    ntStatus = ClearLineCreateControlDevice(DriverObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("ClearLineCreateControlDevice failed, 0x%x", ntStatus)),
        Done);

    DriverObject->MajorFunction[IRP_MJ_CREATE] = ClearLineControlDispatchCreateClose;
    DriverObject->MajorFunction[IRP_MJ_CLOSE] = ClearLineControlDispatchCreateClose;
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = ClearLineControlDispatchDeviceControl;
